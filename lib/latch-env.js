...
const LatchEnv = (() => {
  ...
  const writeConfig = async content =>
    await fs.writeFileSync(
      ...
      err => err && Prompts.static.err(err)
    );
  ...
  const handleProjects = ({
    ...
    configSettings.projects = Object.assign({}, configSettings.projects, {
      [`${alias}`]: {
        isProxy,
        directory: directory
          ? `${configSettings.mainDirectory}/${directory}`
          : `/`,
  ...
  const promptForProjectSettings = async mainDirectory => {
    ...
    prompt(Prompts.userInput.newProject(projects))
      .then(answers => {
        handleProjects(answers);
        if (answers.anotherProject) {
          promptForProjectSettings(mainDirectory);
        }
      })
      .catch(err => err && Prompts.static.error(err));
  };
